...
export default (sheetSpring: Tilesheet, sheetWinter: Tilesheet) => {
  const tiles = [
    [21, 22, 23, 24, 9],
    [28, 29, 30, 31, 9],
    [22, 5, 3, 6, 10],
  ...
  const springSceneCanvas = document.getElementById(
    "scene_sample"
  ...
  const winterSceneCanvas = document.getElementById(
    "scene_sample_winter"
  ) as HTMLCanvasElement;
  ...
  const animatedSceneCanvas = document.getElementById(
    "scene_sample_animation"
  ) as HTMLCanvasElement;
  const myAnimatedScene = new Scene(
    tilesWithSomeAnimation,
    animatedSceneCanvas
  );
