...
class Tilesheet {
  ...
  protected palettedImages: Map<Palette, HTMLImageElement | HTMLCanvasElement> =
    new Map();
  ...
  generatePalettedImage(palette: Palette): HTMLCanvasElement {
    if (!this.image.complete) {
      throw new Error(
        "Tilesheets::generatePalettedImage: image is not fully loaded yet."
      );
    }

    const referencePalette = this.getReferencePalette();
    const {
      canvas: palettedImage,
      ctx,
      imageData,
    } = this.initNewCanvasWithImage();
    ...
    for (let i = 0, l = pixelData.length; i < l; i += 4) {
      ...
      const color = [red, green, blue, alpha] as [
        number,
        number,
        number,
        number
      ...
      const [newRed, newGreen, newBlue, newAlpha] =
        palette.getColorByIndex(index);
  ...
  getImage(paletteToUse?: Palette): HTMLImageElement | HTMLCanvasElement {
    ...
    return this.palettedImages.get(paletteToUse) as
      | HTMLImageElement
      | HTMLCanvasElement;
  }
  ...
  getTileRect(tileIndex: number): Rectangle {
    if (!this.image.complete) {
      throw new Error(
        "Tilesheets::getTileRect: image is not fully loaded yet."
      );
    }

    const { naturalWidth } = this.image;
    const totalTilesOnRow = Math.ceil(
      naturalWidth / (this.tileWidth + this.margin)
    );
  ...
  getTileStyle(tileIndex: number): TileCSSDeclaration {
    if (!this.image.complete) {
      throw new Error(
        "Tilesheets::getTileStyle: image is not fully loaded yet."
      );
    }
  ...
  getTileDomElement(tileIndex: number): HTMLSpanElement {
    if (!this.image.complete) {
      throw new Error(
        "Tilesheets::getTileDomElement: image is not fully loaded yet."
      );
    }
    ...
    Object.keys(tileStyle).forEach((styleProp) => {
      domElement.style[styleProp as keyof TileCSSDeclaration] =
        tileStyle[styleProp as keyof TileCSSDeclaration];
    });
  ...
  getReferencePalette(): Palette {
    if (!this.image.complete) {
      throw new Error(
        "Tilesheets::getReferencePalette: image is not fully loaded yet."
      );
    }
